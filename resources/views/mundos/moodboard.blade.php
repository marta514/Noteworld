...
@php
    // Verificamos si el usuario logueado es el creador del mundo
    $esCreador = auth()->check() && auth()->user()->id === $mundo->user_id;

    // Función "A prueba de balas" para arreglar cualquier formato de ruta
    if (!function_exists('resolverRutaImagen')) {
        function resolverRutaImagen($ruta) {
            // Si ya es una URL completa (http://127... o http://localhost...)
            if (str_starts_with($ruta, 'http://') || str_starts_with($ruta, 'https://')) {
                return $ruta;
            }
            // Si incluye la carpeta (moodboard/imagen.png)
            if (str_starts_with($ruta, 'moodboard/')) {
                return asset('storage/' . $ruta);
            }
            // Si es solo el nombre del archivo
            return asset('storage/moodboard/' . $ruta);
        }
    }
@endphp
...
<div class="flex flex-col md:flex-row gap-6 h-[800px]">
    ...
    <div class="w-full md:w-1/4 bg-old-lace border-2 border-turquoise p-4 rounded-lg overflow-y-auto shadow-inner">
        <h3 class="font-fantasy text-xl text-cerulean mb-4 border-b border-turquoise pb-2">Banco de mundos</h3>
            ...
            @forelse($imagenesApi as $img)
                <form action="{{ route('mundos.moodboard.agregar', $mundo->id) }}" method="POST" class="aspect-square">
                    ...
                    <button type="submit" class="w-full h-full hover:scale-105 transition transform focus:outline-none rounded-lg overflow-hidden shadow-md border-2 border-transparent hover:border-grapefruit">
                        <img src="{{ resolverRutaImagen($img->url) }}" class="w-full h-full object-cover" alt="Imagen API">
                    </button>
                </form>
            @empty
                <p class="text-xs text-gray-500 col-span-2">No hay imágenes de mundos.</p>
            @endforelse
        </div>
    </div>
    @endif
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 content-start">
            @forelse($imagenesGuardadas as $guardada)
                <div class="relative group aspect-square rounded-lg overflow-hidden shadow-lg border border-gray-200">
                    <img src="{{ resolverRutaImagen($guardada->url) }}" class="w-full h-full object-cover transition duration-300 group-hover:scale-110" alt="Imagen en Moodboard">
                </div>
            @empty
                <div class="col-span-full flex flex-col items-center justify-center h-64 text-gray-400">>
                    <p>{{ $esCreador ? 'Tu cuadrícula está vacía. Selecciona imágenes del banco de la API.' : 'Este moodboard aún no tiene imágenes.' }}</p>
                </div>
            @endforelse
        </div>
    </div>
